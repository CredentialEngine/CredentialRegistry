RSpec.describe API::V1::Containers do # rubocop:todo RSpec/MultipleMemoizedHelpers
  ...
  it 'returns 404' do # rubocop:todo Layout/IndentationConsistency
    ...
    expect_status(:not_found)
      ...
      context 'when authenticated and container exists' do # rubocop:todo RSpec/NestedGroups
        ...
        before do
          allow(repository).to receive(:add_member_uri).with(parsed_subresource)
        end

        it 'instantiates a repository and calls add_member_uri with the parsed resource' do
          patch "/containers/#{container_ctid}/resources",
                subresource,
                'Authorization' => "Token #{user.auth_token.value}"

          expect(repository).to have_received(:add_member_uri).with(parsed_subresource)
        end
      end
      # rubocop:enable RSpec/MultipleMemoizedHelpers
    end
    ...
    # rubocop:todo RSpec/MultipleMemoizedHelpers
    describe 'DELETE /containers/:container_ctid/resources' do
      # rubocop:todo RSpec/NestedGroups
      context 'when not authenticated' do # rubocop:todo RSpec/MultipleMemoizedHelpers, RSpec/NestedGroups
        # rubocop:enable RSpec/NestedGroups
        it 'returns 401' do
          delete "/containers/#{container_ctid}/resources", subresource
          expect_status(:unauthorized)
        end
      end
  ...
  it 'returns 404' do # rubocop:todo Layout/IndentationConsistency
    delete "/containers/#{container_ctid}/resources",
           subresource,
           'Authorization' => "Token #{user.auth_token.value}"
    ...
    expect_status(:not_found)
      ...
      context 'when authenticated and container exists' do # rubocop:todo RSpec/NestedGroups
        ...
        before do
          allow(repository).to receive(:remove_member_uris).with(parsed_subresource)
        end

        it 'instantiates a repository and calls remove_member_uris with the parsed resource' do
          delete "/containers/#{container_ctid}/resources",
                 subresource,
                 'Authorization' => "Token #{user.auth_token.value}"

          expect(repository).to have_received(:remove_member_uris).with(parsed_subresource)
        end
      end
      # rubocop:enable RSpec/MultipleMemoizedHelpers
    end
  ...
  it 'returns 404' do # rubocop:todo Layout/IndentationConsistency
    ...
    # rubocop:todo RSpec/MultipleMemoizedHelpers
    describe 'DELETE /:community/containers/:container_ctid/resources' do
      # rubocop:todo RSpec/NestedGroups
      context 'when not authenticated' do # rubocop:todo RSpec/MultipleMemoizedHelpers, RSpec/NestedGroups
        # rubocop:enable RSpec/NestedGroups
        it 'returns 401' do
          delete "/navy/containers/#{container_ctid}/resources", subresource
          expect_status(:unauthorized)
        end
      end
  ...
  it 'returns 404' do # rubocop:todo Layout/IndentationConsistency
    delete "/navy/containers/#{container_ctid}/resources",
           subresource,
           'Authorization' => "Token #{user.auth_token.value}"
