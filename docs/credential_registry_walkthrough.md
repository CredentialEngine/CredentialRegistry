...
```shell
ruby bin/jwt_encode resource.json ~/.ssh/id_rsa > resource_encoded
```

the file `resource_encoded` will contain an encoded string for our resource.
...
```
{
  ...
  "envelope_community": /* The desired community, on our case 'credential_registry' */
  "resource": /* JWT encoded resource, the content from 'resource_encoded' */,
...
If any error occurs you will probably receive an `422 unprocessable_entity`
